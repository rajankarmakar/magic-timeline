...
# Package a buyer-ready release zip at dist/magic-timeline-<version>.zip
...
**Text domain is `magic-timeline`** everywhere — every user-facing string must be wrapped in `__()`/`_e()` with that exact domain, then swept into `languages/magic-timeline.pot` via the `wp i18n make-pot` command above before committing.
